...
def make_relation_agent() -> Agent:
    ...
    return Agent(
        ...
        goal=(
            "Identify 5 key relationships. Output ONLY a list in this exact format:\n"
            "- X: [Column1] | Y: [Column2] | Type: [ChartType]\n"
            "DO NOT output any other text, introductions, or explanations."
        ),
        backstory=(
            "You are a precise Data Analyst. You strictly follow formatting instructions. "
            "You NEVER invent column names. You ONLY output the requested list."
        ),
